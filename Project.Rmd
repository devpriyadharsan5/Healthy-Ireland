...
### 6. Does the proportion of individuals who smoke vary significantly across different age groups?
...
s6$spq1 <- ifelse(s6$spq1 %in% c("Fair", "Bad", "Very Bad"), "Bad", ifelse(s6$spq1 == "Good", "Good", "Very Good"))
...
### 12. What percentage of the population consumes alcohol and how does this vary by counties?
...
### 13. In the last 12 months, how often have you consumed alcoholic beverages and have a private health insurance?

```{r, echo = FALSE, warning=FALSE,message=FALSE}
s13 <- n1[c("q14", "q52", "ageclass", "q55")]
...
1. For males:
   - Among males aged 15-24 years, 18 individuals (1.67%) with private health insurance reported not consuming alcohol, while 43 individuals (3.98%) without private health insurance also reported not consuming alcohol.
  
   - In the 25-44 years age group, 19 individuals (1.76%) with private health insurance and 52 individuals (4.81%) without private health insurance reported not consuming alcohol.
  
   - For males aged 45-64 years, 53 individuals (4.90%) with private health insurance and 65 individuals (6.01%) without private health insurance reported not consuming alcohol.
  
   - Among males aged 65 and over, 62 individuals (5.74%) with private health insurance and 71 individuals (6.57%) without private health insurance reported not consuming alcohol.


2. For females:
   - In the 15-24 years age group, 33 individuals (3.05%) with private health insurance and 47 individuals (4.35%) without private health insurance reported not consuming alcohol.
  
   - Among females aged 25-44 years, 41 individuals (3.79%) with private health insurance and 112 individuals (10.36%) without private health insurance reported not consuming alcohol.
  
   - For females aged 45-64 years, 73 individuals (6.75%) with private health insurance and 122 individuals (11.29%) without private health insurance reported not consuming alcohol.
  
   - Among females aged 65 and over, 99 individuals (9.16%) with private health insurance and 171 individuals (15.82%) without private health insurance reported not consuming alcohol.



### 14. How does folic acid consumption vary across different regions in terms of percentages?
...
### 15. What is the relationship between the prevalence of consumption of alcohol among the surveyed individuals based on their age group and their status as the Chief Income Earner in their household?


```{r, echo = FALSE, warning=FALSE,message=FALSE}
s15 <- n1[c("q14", "ageclass", "q63b")]
...
### 16. How does alcohol consumption vary among the surveyed individuals based on their country of birth?
...
ggplot(drinking_by_country, aes(x = countbirth, y = perc, fill = q14)) +
  ...
  labs(x = "County", y = "Percentage")+
  theme(axis.text.x = element_text(angle = 30,hjust=1))+
  ggtitle ("Alcohol Consumption by Country of Birth")
```
...
Alcohol Consumption Rates by Country of Birth: The table presents the alcohol consumption rates among individuals based on their country of birth. For example, among individuals born in Ireland, 86.53% consume alcohol, while among those born in the UK, the consumption rate is 90.43%. These rates vary based on the country of birth, indicating differences in alcohol consumption behaviors among different populations.

Variation in Alcohol Consumption Rates: The data reveals significant variations in alcohol consumption rates based on the country of birth. For instance, individuals born in the Rest of the World have a lower alcohol consumption rate of 67.37%, while those born in the Rest of EU have a higher rate of 87.29%. These disparities suggest that cultural and demographic factors associated with the country of birth may influence alcohol consumption behaviors.

Comparison by Country of Birth: The table allows for a comparison of alcohol consumption rates among individuals from different countries. For example, individuals born in Ireland have a relatively high alcohol consumption rate of 86.53%, whereas individuals born in the Rest of the World exhibit a lower consumption rate of 67.37%.
...
### 17. How do individuals rate their own health in general, and are there differences between genders and age groups?
...
# converting the values "Fair", "Bad", and "Very Bad" to "Bad" and keeps the rest of the values as "Good" or "Very Good"
s17$spq1 <- ifelse(s17$spq1 %in% c("Fair", "Bad", "Very Bad"), "Bad", ifelse(s17$spq1 == "Good", "Good", "Very Good"))
...
health <- 
  ...
  group_by(spq1) %>% 
  dplyr::summarise(perc = count*100/sum(count),
                   ageclass=ageclass,q52=q52) %>% 
  arrange(spq1)


# Plot the results
ggplot(health, aes(x = spq1, y = perc, fill = spq1)) +
  geom_bar(stat = "identity",position="dodge")  +
  labs(x = "Age Group", y = "Proportion", fill = "Health")+
  ggtitle("Proportion of Individuals Rating Their Health by Gender and Age Group")+
  facet_wrap(~q52+ageclass)

```


Based on the provided data, we can analyze how individuals rate their own health condition (variable: spq1) across different age groups (variable: ageclass) and genders (variable: q52).

1. Proportion of Individuals Rating Their Health Condition by Gender and Age Group: The table presents the proportions of individuals rating their health condition as "Bad," "Good," or "Very Good" based on their gender and age group. The percentages are calculated within each subgroup.

2. Differences by Gender: Among males, the proportion of individuals rating their health as "Bad" is highest in the 65 and over age group (23.56%), followed by the 45-64 years age group (17.56%). For females, the highest proportion of individuals rating their health as "Bad" is observed in the 65 and over age group (23.33%), followed by the 45-64 years age group (16.74%).

3. Differences by Age Group: Within the male population, the 65 and over age group has the highest proportion of individuals rating their health as "Bad" (23.56%), followed by the 45-64 years age group (17.56%). Among females, the 65 and over age group also has the highest proportion of individuals rating their health as "Bad" (23.33%), followed by the 45-64 years age group (16.74%).

4. Overall Health Ratings: Across all gender and age groups, the proportions of individuals rating their health as "Good" or "Very Good" are generally higher compared to those rating their health as "Bad." The highest proportions of individuals rating their health as "Good" or "Very Good" are observed in the 25-44 years age group for both males and females.

These findings indicate variations in self-rated health across different age groups and genders. Older age groups tend to have a higher proportion of individuals rating their health as "Bad" compared to younger age groups. Further analysis and exploration can provide insights into the factors influencing self-rated health, such as underlying health conditions, lifestyle factors, and access to healthcare.
