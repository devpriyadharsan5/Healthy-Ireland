...
n1$lho <- ifelse(n1$lho %in% c("Carlow/Kilkenny"), "Carlow/Kilkenny",
                 ...
                 ifelse(n1$lho %in% c("Kildare/West Wicklow"), "Kildare/West Wicklow",
...
example_dat1 <- n1 %>% dplyr::select(q14,ageclass,q52,key1)
...
logistic_mod1 <- glm(q14_binary ~ ageclass + q52+key1, data = train_data1, family = "binomial")


# Fit random forest model
random_forest1 <- randomForest(q14_binary ~ ageclass + q52+key1, data = train_data1, ntree = 100)


# Fit decision tree model
decision_tree1 <- rpart(q14_binary ~ ageclass + q52+key1, data = train_data1, method = "class")


# Fit Naive Bayes model
naive_bayes1 <- naive_bayes(q14_binary ~ ageclass + q52+key1, data = train_data1)


# Fit SVM model
svm_model1 <- svm(q14_binary ~ ageclass + q52+key1, data = train_data1, kernel = "linear")
