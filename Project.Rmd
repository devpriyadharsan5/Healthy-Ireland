...
```{r, echo = FALSE, warning=FALSE,message=FALSE}
# Recode values in column q14 to "Yes" or "No"
...
# Filter out the records with "Don’t know/refused" in the "q120" column
suppl <- suppl %>%
  filter(q120 != "Don’t know/refused")


# Recode the values in the "q120" column to combine "No", "Don’t know/refused" as "No" and keep the rest as "Yes"
...
decision_tree <- rpart(q6_binary ~ ageclass + q52+key1+lho+q120, data = train_data, method = "class")

# Fit Naive Bayes model
naive_bayes <- naive_bayes(q6_binary ~ ageclass + q52+key1+lho+q120, data = train_data)

# Fit SVM model
svm_model <- svm(q6_binary ~ ageclass + q52+key1+lho+q120, data = train_data, kernel = "linear")
...
```{r,echo = FALSE, warning=FALSE,message=FALSE}
library(pscl)
pscl::pR2(logistic_mod)["McFadden"]
...
```{r,echo = FALSE, warning=FALSE,message=FALSE}
caret::varImp(logistic_mod)

car::vif(logistic_mod)
...
predicted_probs <- predict(logistic_mod, newdata = test_data, type = "response")

# Create ROC curve
roc_curve <- roc(test_data$q6_binary, predicted_probs)

# Plot ROC curve
plot(roc_curve, main = "ROC Curve for Logistic Regression Model",
     col = "blue", lwd = 2)

# Add 45-degree diagonal line for reference (random classifier)
abline(a = 0, b = 1, lty = 2)

# Add text with AUC (Area Under the Curve) value
auc_val <- auc(roc_curve)
text(0.7, 0.3, paste("AUC =", round(auc_val, 2)), col = "blue", cex = 1.2)

# Add legend
legend("bottomright", legend = c("Logistic Model", "Random Classifier"),
       col = c("blue", "black"), lty = c(1, 2), lwd = c(2, 1),
       cex = 0.8, bty = "n")

```
```{r,echo = FALSE, warning=FALSE,message=FALSE}
hist(residuals(logistic_mod), col = "steelblue")
```
```{r,echo = FALSE, warning=FALSE,message=FALSE}
#create fitted value vs residual plot
plot(fitted(logistic_mod), residuals(logistic_mod))
...
log.model <- glm(q6_binary ~., data = example_dat, family = binomial(link = "logit"))
summary(log.model)
```
...
```{r,echo = FALSE, warning=FALSE,message=FALSE}
table(log.prediction.rd, test[,7])


accuracy <- table(log.prediction.rd, test[,7])
...
predicted_classes_dt <- predict(decision_tree, newdata = test_data, type = "class")
...
predicted_classes_nb <- predict(naive_bayes, newdata = test_data)

# Create the confusion matrix
confusion_matrix <- confusionMatrix(predicted_classes_nb, test_data$q6_binary)
...
# Create the confusion matrix
confusion_matrix <- confusionMatrix(predicted_classes_svm, test_data$q6_binary)
...
# Fit decision tree model
decision_tree1 <- rpart(q14_binary ~ ageclass + q52+key1+lho+q330_1+q330_2, data = train_data1, method = "class")

# Fit Naive Bayes model
naive_bayes1 <- naive_bayes(q14_binary ~ ageclass + q52+key1+lho+q330_1+q330_2, data = train_data1)

# Fit SVM model
svm_model1 <- svm(q14_binary ~ ageclass + q52+key1+lho+q330_1+q330_2, data = train_data1, kernel = "linear")
...
# Evaluate the model on the test dataset
predicted_classeslog1 <- ifelse(predict(logistic_mod1, newdata = test_data1, type = "response") > 0.5, "Yes", "No")
...
# Create the confusion matrix
confusion_matrix <- confusionMatrix(predicted_classeslog1, test_data1$q14_binary)
...
```{r,echo = FALSE, warning=FALSE,message=FALSE}
# Predict the classes using the decision tree model
predicted_classesdt1 <- predict(decision_tree1, newdata = test_data1, type = "class")

# Create the confusion matrix
confusion_matrix1 <- confusionMatrix(predicted_classesdt1, test_data1$q14_binary)

# Calculate accuracy
accuracy <- round(confusion_matrix1$overall["Accuracy"], digits = 4)

# Print the accuracy and confusion matrix
print(paste("Accuracy (Decision Tree):", accuracy * 100))
...
```{r,echo = FALSE, warning=FALSE,message=FALSE}
# Predict the classes using the Naive Bayes model
predicted_classesnb1 <- predict(naive_bayes1, newdata = test_data1)

# Create the confusion matrix
confusion_matrix1 <- confusionMatrix(predicted_classesnb1, test_data1$q14_binary)
...
```{r,echo = FALSE, warning=FALSE,message=FALSE}
# Predict the classes using the SVM model
predicted_classessvm1 <- predict(svm_model1, newdata = test_data1)

# Create the confusion matrix
confusion_matrix1 <- confusionMatrix(predicted_classessvm1, test_data1$q14_binary)
