...
```{r}
n1$lho[n1$lho == "Carlow/Kilkenny"] <- "Carlow"
n1 <- rbind(n1, n1[n1$lho == "Carlow", ])
n1$lho[n1$lho == "Carlow"] <- "Kilkenny"
...
n1$lho <- ifelse(n1$lho %in% c("Carlow/Kilkenny"), "Carlow/Kilkenny",
                 ...
                 ifelse(n1$lho %in% c("Sligo/Leitrim/West Cavan"), "Sligo/Leitrim/West Cavan",
...
```{r,echo = FALSE, warning=FALSE,message=FALSE}
ireland_counties <- st_read("counties.shx")
st_layers("counties.shx")
...
example_dat <- n1 %>% dplyr::select(q6, ageclass, q52,key1,lho)
...
set.seed(123)
trainIndex <- createDataPartition(example_dat$q6_binary, p = 0.65, list = FALSE)
train_data <- example_dat[trainIndex, ]
test_data <- example_dat[-trainIndex, ]

# Fit logistic regression model
logistic_mod <- glm(q6_binary ~ ageclass + q52+key1+lho, data = train_data, family = "binomial")

# Fit decision tree model
decision_tree <- rpart(q6_binary ~ ageclass + q52+key1+lho, data = train_data, method = "class")

# Fit Naive Bayes model
naive_bayes <- naive_bayes(q6_binary ~ ageclass + q52+key1+lho, data = train_data)

# Fit SVM model
svm_model <- svm(q6_binary ~ ageclass + q52+key1+lho, data = train_data, kernel = "linear")
...
example_dat1 <- n1 %>% dplyr::select(q14,ageclass,q52,key1,lho)
...
# Create a training and testing dataset
set.seed(123)
trainIndex1 <- createDataPartition(example_dat1$q14_binary, p = 0.65, list = FALSE)
train_data1 <- example_dat1[trainIndex1, ]
test_data1 <- example_dat1[-trainIndex1, ]


# Fit the logistic regression model
logistic_mod1 <- glm(q14_binary ~ ageclass + q52+key1+lho, data = train_data1, family = "binomial")

# Fit decision tree model
decision_tree1 <- rpart(q14_binary ~ ageclass + q52+key1+lho, data = train_data1, method = "class")

# Fit Naive Bayes model
naive_bayes1 <- naive_bayes(q14_binary ~ ageclass + q52+key1+lho, data = train_data1)

# Fit SVM model
svm_model1 <- svm(q14_binary ~ ageclass + q52+key1+lho, data = train_data1, kernel = "linear")
...
```{r,echo = FALSE, warning=FALSE,message=FALSE}
# Predict the classes using the Naive Bayes model
predicted_classes1 <- predict(naive_bayes1, newdata = test_data1)

# Create the confusion matrix
confusion_matrix1 <- confusionMatrix(predicted_classes1, test_data1$q14_binary)
...
```{r,echo = FALSE, warning=FALSE,message=FALSE}
# Predict the classes using the SVM model
predicted_classes1 <- predict(svm_model1, newdata = test_data1)
...
# Evaluate the model on the test dataset
predicted_classes1 <- ifelse(predict(logistic_mod1, newdata = test_data1, type = "response") > 0.5, "Yes", "No")

# Convert test_data$q6_binary to a factor with the same levels as predicted_classes
test_data1$q14_binary <- factor(test_data1$q14_binary, levels = c("Yes", "No"))

# Convert predicted_classes to a factor with the same levels as test_data$q6_binary
predicted_classes1 <- factor(predicted_classes1, levels = c("Yes", "No"))

# Create the confusion matrix
confusion_matrix <- confusionMatrix(predicted_classes, test_data1$q14_binary)
...
```{r,echo = FALSE, warning=FALSE,message=FALSE}
# Predict the classes using the decision tree model
predicted_classes1 <- predict(decision_tree1, newdata = test_data1, type = "class")
