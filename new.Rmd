...
n3= n1|> group_by(q6,q52) |> 
  dplyr::summarise(count=n()) |> 
  ungroup() |> group_by(q6) |> 
  dplyr::summarise(perc=sum(q6 == "Yes,")/n()*100, q52=q52) |>
  drop_na()

# create a new dataframe with smoking and gender variables only
#smoking_gender <- n1 |> select(q6, q52)|> drop_na()

# calculate the percentage of smokers by gender
#smoking_gender_perc <- smoking_gender %>% group_by(q52) %>% 
                       #summarize(smokers_perc = sum(q6 == "Daily")/n()) 


# create a bar plot
ggplot(data = n3, aes(x = q52, y = perc, fill =q6)) + 
  geom_bar(stat = "identity", position="dodge") +
  labs(x = "Gender", y = "Percentage of smokers", title = "Smoking prevalence by gender")
...
ggplot(data=n3, aes(x=q52, y=perc, fill=q6)) + 
  geom_bar(stat="identity", position="dodge") +
  labs(x="Gender", y="Percentage", fill="Smoking") + 
  scale_fill_manual(values=c("gray80", "gray30"), labels=c("No", "Yes,")) +
  ggtitle("Percentage of Smokers by Gender and Age") 
```
